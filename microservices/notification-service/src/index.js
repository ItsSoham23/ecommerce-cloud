...
app.listen(PORT, async () => {
  ...
  try {
    // Start Kafka consumer only when explicitly enabled. This prevents the service
    // from crashing or blocking startup when Kafka is not available (e.g. in
    // local dev or during partial cluster outages).
    if (process.env.USE_KAFKA === 'true') {
      ...
      // start consumer but ensure any internal errors are caught to avoid
      // unhandled rejections from killing the process
      consumer.start(async (message) => {
        ...
        console.error('Kafka consumer failed to start', err && err.message ? err.message : err);
      });
    } else {
      console.log('USE_KAFKA not set to "true"; skipping Kafka consumer startup');
    }
  } catch (e) {
    console.error('Kafka consumer setup error', e && e.message ? e.message : e);
  }
});
